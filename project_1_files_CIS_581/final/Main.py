...
year_train = np.loadtxt(r"C:\Users\SS Studios\Desktop\UMD_WINTER_2023\COURSES\CL\Mid project\project_1_files_CIS_581\final\train.dat", usecols=(0), unpack=True)
indicator_train = np.loadtxt(r"C:\Users\SS Studios\Desktop\UMD_WINTER_2023\COURSES\CL\Mid project\project_1_files_CIS_581\final\train.dat", usecols=(1), unpack=True)
...
def best_degree(X,y):
    ...
    plt.show()
    print("best_lambda(regularization_factor) : {} and its RMSE {}".format(degree_df[degree_df["avg RMSE"] == degree_df["avg RMSE"].min()].d[5],degree_df["avg RMSE"].min()))
    print("")
    return degree_df
...
def best_lambda(X,y):
    ...
    for l in lambda_:
        # Initialize a polynomial regression model pipeline with scaling
        model = make_pipeline(StandardScaler(),PolynomialFeatures(degree = 12, include_bias = True),StandardScaler(), Ridge(alpha=l,fit_intercept=True,solver='cholesky'))
...
def entire_training_rmse(X,y,degree,l):
    regr = make_pipeline(StandardScaler(),PolynomialFeatures(degree, include_bias = True), Ridge(alpha=l,fit_intercept=True,solver='cholesky'))
    regr.fit(X, y)
...
def entire_test_rmse(X,X_test,y,y_test,degree,l):
    ...
    plt.legend(['true data',"d={} curve".format(degree),"predicted data"])
    plt.title("polynomial curve d = {} and alpha = exp(-3) fitted in training data".format(degree))
    plt.show()
